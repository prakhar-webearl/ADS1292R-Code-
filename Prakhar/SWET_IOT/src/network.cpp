...
static void uploadTask(void* pv);
 static void localStatusTask(void* pv);   // HTTP callbacks to phone app
...
// -----------------------------------------------------------
// Local status notification constants
// The ESP32 sends HTTP GET to the phone's local server whenever WiFi
// state changes. The phone app runs a simple HTTP server on port 9000
// (same mechanism as the working ecg_esp32_ads1292r_v10 code).
// Endpoints:
//   GET /device-status?connected=true&deviceId=<id>   → WiFi just connected
//   GET /device-status?connected=false&deviceId=<id>  → WiFi just dropped
//   GET /heartbeat?deviceId=<id>                      → periodic keep-alive
// -----------------------------------------------------------
#define LOCAL_APP_PORT              9000
...
// AP mode
// -----------------------------------------------------------
// Idle Mode (AP + STA Reconnecting + BLE Advertising)
// -----------------------------------------------------------
void network_startIdleMode() {
    WiFi.disconnect(true, false);
    WiFi.softAPdisconnect(true);
    delay(50);
    // Always use AP_STA so both Hotspot and Router connection are available
    WiFi.mode(WIFI_AP_STA);
    bool ok = WiFi.softAP(WIFI_AP_SSID, WIFI_AP_PASSWORD);
    if (!ok) {
        WiFi.mode(WIFI_OFF); delay(120); WiFi.mode(WIFI_AP_STA);
        ok = WiFi.softAP(WIFI_AP_SSID, WIFI_AP_PASSWORD);
    }
    if (ok) {
        s_apActive     = true;
        s_setupApMode  = true;
        Serial.print(F("[NET] Idle Mode: AP started. SSID=")); Serial.print(WIFI_AP_SSID);
        Serial.print(F("  IP=")); Serial.println(WiFi.softAPIP());
    } else {
        Serial.println(F("[NET] AP start FAILED."));
    }
    
    // Ensure BLE is advertising so previous users can auto-connect
    ble_startAdvertising();
    
    // Force STA to attempt reconnecting immediately (if credentials exist)
    s_lastReconnectMs = 0; 
...
static void handleWifiConfig() {
    ...
    // Notify mobile via BLE that we are about to try connecting
    ble_sendStatus(F("{\"status\":\"wifi_connecting\"}"));
    ...
    if (ok) {
        ...
        WiFi.softAPdisconnect(true);
        WiFi.mode(WIFI_STA);
        // Do NOT call ble_stopAdvertising() here. Let BLE remain available.
        ledOn();
        Serial.print(F("[NET] STA connected via config. IP="));
        Serial.println(WiFi.localIP());

        // ── Push WiFi success over BLE ─────────────────────────────────────
        // The mobile app subscribes to the Status characteristic (UUID ...abcdef2).
        // This notification tells it that WiFi credentials were accepted and the
        // IP address the device got — no polling or timeout needed on the app side.
        String statusJson = "{\"status\":\"wifi_connected\",\"ip\":\"";
        statusJson += WiFi.localIP().toString();
        statusJson += "\",\"deviceId\":\"";
        statusJson += String(s_deviceId);
        statusJson += "\",\"userId\":\"";
        statusJson += String(s_userId);
        statusJson += "\"}";
        ble_sendStatus(statusJson);
        Serial.println(F("[NET] BLE status push: wifi_connected"));
    } else {
        Serial.println(F("[NET] Connect failed — restoring Idle Mode."));
        // Notify mobile that connection failed
        ble_sendStatus(F("{\"status\":\"wifi_failed\"}"));
...
void network_init() {
    ...
    // -------------------------------------------------------
    WiFi.mode(WIFI_STA);
    WiFi.setSleep(true);  // CRITICAL: Must be true when BLE is also active!
    ...
    if (loadCredentials(ssid, pass)) {
        ...
        Serial.println(F("[NET] No saved credentials."));
    }

    // Determine boot priority based on last_mode
    s_prefs.begin(NVS_NAMESPACE, true);
    String lastMode = s_prefs.getString("last_mode", "");
    s_prefs.end();

    if (lastMode == "ble") {
        Serial.println(F("[NET] last_mode was 'ble'. Giving BLE directed advertising priority for 30s."));
        // We do NOT start Idle Mode yet. ble_init() (called by main) will start directed adv.
        // If it times out, ble_update() will call network_startIdleMode().
    } else {
        Serial.println(F("[NET] last_mode was 'wifi' or empty. Entering Idle Mode."));
        network_startIdleMode();
    }
    ...
    // Local status task: notifies the phone app over WiFi when connection state
    // changes. Mirrors the localStatusTask from ecg_esp32_ads1292r_v10_working_1.
    xTaskCreatePinnedToCore(localStatusTask, "local_status", 4096, nullptr, 1, nullptr, 0);
    Serial.printf("[NET] Local status task started (port %d).\n", LOCAL_APP_PORT);
}
...
void network_update() {
    ...
    ble_update();              // Restart BLE advertising if phone disconnected (deferred from callback)
    ...
    // --- Exclusive Transport Logic ---
    if (bleConnected && !wasBleConnected) {
        Serial.println(F("[NET] BLE Connected! Stopping AP hotspot (STA kept if active)."));
        // Only stop the AP hotspot — do NOT disconnect STA.
        // Calling WiFi.disconnect() here would kill STA and user would
        // need to reconfigure WiFi on second BLE connect. Bug fix #2.
        WiFi.softAPdisconnect(true);
        s_apActive = false;
        wasBleConnected = true;
    } else if (!bleConnected && wasBleConnected) {
        wasBleConnected = false;
        // BLE LED off immediately — regardless of what path we take below. Bug fix #3.
        ledOff();
        ...
        if (lastMode == "ble") {
            Serial.println(F("[NET] BLE Disconnected. Giving BLE 30s priority before restoring WiFi..."));
            // Do NOTHING more here. ble_update() will start priority advertising;
            // on 30s timeout it calls network_startIdleMode() for us.
        } else {
            Serial.println(F("[NET] BLE Disconnected. Restoring Idle Mode (AP + STA + BLE)..."));
            network_startIdleMode();
        }
    }

    if (bleConnected || ble_isPriorityAdv()) {
        if (bleConnected) ledOn();
        return; // Do not process WiFi state machine while BLE is the active exclusive transport or has priority
    }
    ...
    if (currentlyConnected && !s_staConnected) {
        ...
        // Save priority to WiFi
        s_prefs.begin(NVS_NAMESPACE, false);
        s_prefs.putString("last_mode", "wifi");
        s_prefs.end();

        // Do NOT call ble_stopAdvertising() here. 
        // We want BLE to remain available for auto-connect even if WiFi is connected to a router.
        // If a BLE client connects, THEN WiFi will be shut down.
    }
    ...
    if (!currentlyConnected && s_staConnected) {
        ...
        s_staLostTimeMs = now;
        ledOff();
        Serial.println(F("[NET] STA disconnected. Restoring Idle Mode..."));
        // Notify mobile that WiFi dropped
        ble_sendStatus(F("{\"status\":\"wifi_disconnected\"}"));
        network_startIdleMode();
    }
    ...
    if (!s_staConnected && s_reconnectEnabled && s_hasCredentials) {
        ...
        if ((now - s_lastReconnectMs) >= WIFI_RECONNECT_INTERVAL_MS) {
            ...
            if (ok) {
                ...
                // Bug fix #1: Only start AP once when threshold is first hit, not every loop cycle.
                // Guard with !s_setupApMode to prevent repeated softAP() calls which toggle the hotspot.
                if (s_reconnectFails >= RECONNECT_FAIL_LIMIT && !s_setupApMode) {
                    ...
                    s_apActive    = true;
                    s_setupApMode = true;
                    // Do NOT change s_reconnectFails here — let it keep incrementing
                    // so we don't ever re-enter this block while still failing.
                }
                // Saturate to prevent overflow after AP is already started
                if (s_reconnectFails >= 254) s_reconnectFails = RECONNECT_FAIL_LIMIT;
            }
        }
    }

    // LED reflects connection state — ON=connected, OFF=not connected
    if (!bleConnected) {
        if (s_staConnected) ledOn(); else ledOff();
    }
}
...
bool network_uploadBlock(const Block& blk, bool leadsOff, bool loPlus, bool loMinus, const char* warning, const char* severity) {
    ...
    Serial.print(F("#DB_PAYLOAD -> mode=")); Serial.print(p.mode);
    Serial.print(F(" | seq=")); Serial.print(blk.seq);
...
// localStatusTask()
// -----------------------------------------------------------
// Runs on Core 0. Polls WiFi state every 500 ms and makes HTTP GET
// requests to the phone app's local HTTP server whenever the state
// changes or on a heartbeat interval.
//
// Why this is needed:
//   - After /wifi-config, the app already gets a BLE push.
//   - But if the user opens the app AFTER the device connected (e.g.
//     the device auto-connected on boot using saved credentials), the
//     app has no BLE characteristic to read — it needs to know over WiFi.
//   - The heartbeat tells the app the device is alive and connected.
//
// Phone app must listen on port 9000:
//   GET /device-status?connected=true&deviceId=<id>   → connection event
//   GET /device-status?connected=false&deviceId=<id>  → disconnect event
//   GET /heartbeat?deviceId=<id>                      → keep-alive every 5s
//
// Gateway IP is the phone's hotspot IP (default 192.168.43.1 on Android,
// 172.20.10.1 on iPhone hotspot). ESP32 reads it via WiFi.gatewayIP().
// -----------------------------------------------------------
static void localStatusTask(void* pv) {
    ...
    for (;;) {
        ...
        if (isConnected != wasConnected) {
            ...
            Serial.printf("[NET:local] WiFi state → %s\n",
                          isConnected ? "CONNECTED" : "DISCONNECTED");

            IPAddress gw = WiFi.gatewayIP();
            if ((uint32_t)gw != 0) {
                String url = String("http://") + gw.toString()
                           + ":" + String(LOCAL_APP_PORT)
                           + "/device-status?connected=" + (isConnected ? "true" : "false")
                           + "&deviceId=" + String(s_deviceId)
                           + "&userId="   + String(s_userId)
                           + "&ip="       + (isConnected ? WiFi.localIP().toString() : "");

                HTTPClient http;
                http.setTimeout(1500);
                http.begin(url);
                int code = http.GET();
                Serial.printf("[NET:local] /device-status → HTTP %d\n", code);
                http.end();
            }
        }

        // ── Heartbeat (every LOCAL_HEARTBEAT_INTERVAL_MS while connected) ─
        if (isConnected) {
            unsigned long now = millis();
...
// -----------------------------------------------------------
void network_buildJsonPayload(const Block& blk, bool leadsOff, bool loPlus, bool loMinus, const char* warning, const char* severity, const char* mode, String& out) {
    UploadPayload p;
    p.blk      = blk;
    p.leadsOff = leadsOff;
    p.loPlus   = loPlus;
