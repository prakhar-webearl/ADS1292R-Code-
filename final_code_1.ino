...
void applyLowPassToBlock(uint16_t* data, int len) {
  for (int i = 0; i < len; i++) {
    float x = (float)data[i];
    float w = x - lp_a1 * lp_s1 - lp_a2 * lp_s2;
    float y = lp_b0 * w + lp_b1 * lp_s1 + lp_b2 * lp_s2;
    lp_s2 = lp_s1;
    lp_s1 = w;
...
void applySignalFiltersToBlock(Block& blk) {
  ...
  // applyHighPassToBlock(blk.data, WINDOW_SIZE);
  applyLowPassToBlock(blk.data, WINDOW_SIZE);
  applyNotchToBlock(blk.data, WINDOW_SIZE);

  Serial.println("[FILTER] seq=" + String(blk.seq) +
                 " applied: HP 0.5Hz -> LP 40Hz -> Notch 50Hz | ECG 0.5-40Hz preserved | 0.05-0.3Hz drift and 50-200Hz noise reduced");
}
