"""
ECG Live Monitor — v6.2 (ADS1292R Edition) — ULTRA FINE PQRST ALIGNMENT
============================================
Data source : GET https://ads1292r-code.onrender.com/api/ecg/live/ESP_ECG_123

Fine-tuning updates:
 - P-wave window shifted right (180ms start instead of 200ms) to center on P-peak.
 - Q-wave window narrowed (32ms–8ms before R) to perfectly catch the tiny Q-dip.
 - S-wave window extended (0–48ms after R) to find the true lowest S-point.
 - T-wave window start delayed (200ms after R instead of 180ms) to center on T-peak.
"""
...
# ============================================================
# PQRST ANNOTATION — Precision tuned for 125 SPS
# ============================================================
def _annotate_one_beat(ax, sig, r_peak):
    ...
    # P wave: 180–40ms before R (approx 22–5 samples) -> shifted right to catch P peak
    p_lo = max(0, r_peak - int(0.180 * FS))
    ...
    # Q wave: 32–8ms before R (approx 4–1 samples) -> narrows window to the exact dip before R
    q_lo = max(0, r_peak - int(0.032 * FS))
    q_hi = max(0, r_peak - int(0.008 * FS))
    if q_hi > q_lo:
    ...
    # S wave: 0–48ms after R (approx 6 samples) -> extended to find true S minimum
    s_lo = r_peak
    ...
    # T wave: 200–400ms after R (25–50 samples) -> delayed start to push marker slightly right
    t_lo = min(n, r_peak + int(0.200 * FS))
...
print("=" * 65)
print("ECG LIVE MONITOR  v6.2  --  ADS1292R Edition  |  ULTRA FINE PQRST")
print("=" * 65)
