"""
ECG Live Monitor — v6.1 (ADS1292R Edition) — FIXED PQRST TIMING
============================================
Data source : GET https://ads1292r-code.onrender.com/api/ecg/live/ESP_ECG_123

Key fixes:
 - P-wave window: 200–40 ms before R (25–5 samples) → catches true P peak.
 - Q-wave window: 40–20 ms before R (5–2.5 samples) → captures Q minimum.
 - S-wave window: 0–30 ms after R (0–4 samples) → captures S minimum.
 - T-wave window: 180–400 ms after R (22–50 samples) → captures T peak.
 - P/T use absolute deviation from baseline (works for upright/inverted).
 - Q/S use local extrema within tight windows.
 - Baseline filter: 0.67 Hz high-pass + gentle median smoothing.
 - Proper `iso` baseline calculation per beat.
 - Y-axis labels fixed for 24-bit ADC scale.
"""
...
# ============================================================
# SIGNAL FILTERING — Improved baseline removal
# ============================================================
class ECGFilter:
    ...
    def _build(self):
        nyq = self.nyquist
        # 0.67 Hz high-pass (slightly stronger baseline removal than 0.5 Hz)
        self.b_hp, self.a_hp       = butter(2, 0.67 / nyq, btype='high')
        # 40 Hz low-pass (removes EMG/muscle noise)
    ...
    def filter_signal(self, raw_signal):
        ...
        try:
            ...
            sig = uniform_filter1d(sig, size=3)  # Gentle smoothing
        except Exception:
...
# ============================================================
# PQRST ANNOTATION — Fixed timing windows for 125 SPS
# ============================================================
def _annotate_one_beat(ax, sig, r_peak):
    ...
    # Isoelectric baseline for THIS beat (PR/TP segment before it)
    ...
    # P wave: 200–40ms before R (25–5 samples) — uses absolute deviation from baseline
    p_lo = max(0, r_peak - int(0.200 * FS))
    p_hi = max(0, r_peak - int(0.040 * FS))
    if p_hi > p_lo:
        p_seg = sig[p_lo:p_hi]
        ...
        ax.annotate('P', xy=(p_idx, p_val),
                    xytext=(p_idx, p_val + 0.12 * ptp),
                    fontsize=11, fontweight='bold', color='green', ha='center')

    # Q wave: 40–20ms before R (5–2.5 samples) — local minimum
    q_lo = max(0, r_peak - int(0.040 * FS))
    q_hi = max(0, r_peak - int(0.020 * FS))
    if q_hi > q_lo:
    ...
    # S wave: 0–30ms after R (0–4 samples) — local minimum
    s_lo = r_peak
    s_hi = min(n, r_peak + int(0.030 * FS))
    if s_hi > s_lo:
    ...
    # T wave: 180–400ms after R (22–50 samples) — uses absolute deviation from baseline
    t_lo = min(n, r_peak + int(0.180 * FS))
    t_hi = min(n, r_peak + int(0.400 * FS))
    if t_hi > t_lo:
        t_seg = sig[t_lo:t_hi]
...
# ============================================================
# DETECTION ENGINE — Unchanged from original (works well)
# ============================================================
def _preprocess(sig):
    nyq = FS / 2.0
...
print("=" * 65)
print("ECG LIVE MONITOR  v6.1  --  ADS1292R Edition  |  FIXED PQRST")
print("=" * 65)
