...
#define BL_MAX_STEP_PER_CAND 150.0f                            // ADC counts/200ms slew limit — relaxed from 80 to allow real T-wave recovery without being clamped (was causing artificial T inversion)
#define BL_CONTAM_PTP_THRESHOLD 500.0f                        // ADC counts peak-to-peak within one 200ms stage-1 chunk. Raised from 250 to stop absorbing T-wave deflections as "baseline" — only genuine QRS transients exceed 500 counts in 200ms at this scale
...
void loop() {
  ...
  if (newSample) {
    ...
    int32_t scaled;
    if (!lo) {
      // sDaqVals[1] = ECG channel (CH2) as signed 24-bit value.
      // Shift right by 11 bits to preserve more resolution than the
      // previous >>8 + /16 (which was >>12 effective, losing 12 bits).
      // >>11 keeps ~13 bits of useful resolution, giving ±4096 raw range.
      // Center at 2048 to fit the 0-4095 pipeline.
      // Typical ±1mV ECG with gain=6 → ±~4000 raw counts after >>11
      // → centered at 2048, occupies roughly 1000-3000 range = good.
      int32_t ecgRaw = ecgSensorValues.sDaqVals[1];
      // Sign-extend 24-bit to 32-bit (library may already do this)
      if (ecgRaw & 0x800000) ecgRaw |= 0xFF000000;
      scaled = (ecgRaw >> 11) + 2048;
      if (scaled > 4095) scaled = 4095;
      if (scaled < 0)    scaled = 0;
    } else {
      scaled = 0; // leads-off: zero sample signals downstream
    }
