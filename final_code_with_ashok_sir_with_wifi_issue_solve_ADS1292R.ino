/*
 * ESP32 ECG v10.0 — ADS1292R Migration (Full)
...
#include <SPI.h>
#include "ads1292r.h" // ProtoCentral ADS1292R library
...
// ADS1292R library object and shared ECG sample variable.
// latestEcgValue is written inside the DRDY ISR and read in the
// timer ISR. The portMUX below makes both accesses atomic on the
// same core (both run on core 1 with the same priority — the DRDY
// ISR cannot pre-empt the timer ISR on the same core, but the mux
// makes intent clear and guards against future core-migration).
...
// ============================================================
// ADS1292R DRDY INTERRUPT SERVICE ROUTINE
// ============================================================
// Called on every falling edge of DRDY_PIN — the ADS1292R pulses
// DRDY low once per completed conversion at the configured output
// data rate (set to 500 SPS in ads1292rInit() which is hardware-
// decimated down to 360Hz by the timer ISR that reads latestEcgValue).
//
// 9 bytes = 3 status bytes (LOFF_STAT, GPIO, reserved) + 3 bytes CH1
// + 3 bytes CH2.  CH1 is the impedance-pneumography channel; CH2 is
// the ECG channel (ProtoCentral default wiring).
//
// Signed 24-bit → 12-bit pipeline conversion:
//   1. Reconstruct int32 from 3 bytes with manual sign-extension.
//   2. Right-shift by 11 (not 12): ADS1292R with gain=6 on a ±1 mV
//      ECG signal uses roughly bits 10-17 of the 24-bit word.  Shifting
//      right by 11 maps those to bits 0-7, then adding 2048 centers
//      the isoelectric baseline at pipeline midscale.  Shift by 12
//      discards 1 more useful bit and halves the amplitude unnecessarily.
//   3. Clamp to [0, 4095] to guarantee uint16 safety.
//
// Lead-off: status byte 0 carries LOFF_STAT bits.
//   Bit 1 = IN2P (RA electrode, LO+); Bit 0 = IN2N (LA electrode, LO-).
//   Bit 5 = IN1P (RL drive detect) — ignored here.
void IRAM_ATTR onDRDY() {
  uint8_t spiData[9];

  digitalWrite(ADS1292_CS_PIN, LOW);
  for (int i = 0; i < 9; i++) {
    spiData[i] = SPI.transfer(0x00);
  }
  digitalWrite(ADS1292_CS_PIN, HIGH);

  // --- Lead-off detection from LOFF_STAT (status byte 0, bits 3:0) ---
  // ADS1292R datasheet Table 17: bits [3] IN2P, [2] IN2N (for 2-electrode setup)
  uint8_t loffStat = spiData[0] & 0x0F;
  bool newLoPlusOff  = (loffStat & 0x08) != 0;  // IN2P = RA (LO+)
  bool newLoMinusOff = (loffStat & 0x04) != 0;  // IN2N = LA (LO-)

  portENTER_CRITICAL_ISR(&ecgValueMux);
  loPlusOff  = newLoPlusOff;
  loMinusOff = newLoMinusOff;
  leadsOff   = (newLoPlusOff || newLoMinusOff);

  if (!leadsOff) {
    // CH2 ECG data in bytes [6], [7], [8]
    int32_t raw = ((int32_t)spiData[6] << 16)
                | ((int32_t)spiData[7] <<  8)
                |  (int32_t)spiData[8];

    // Sign-extend 24-bit two's complement to full int32
    if (raw & 0x00800000) raw |= 0xFF000000;

    // Scale to pipeline range: shift right 11 then center at 2048.
    // This keeps the ECG swing in roughly ±200-400 counts around 2048,
    // which is well within [0,4095] and gives the peak-detector enough
    // dynamic range without clipping P or T waves.
    int32_t scaled = (raw >> 11) + 2048;
    if (scaled > 4095) scaled = 4095;
    if (scaled <    0) scaled = 0;
    latestEcgValue = scaled;
    newSampleAvailable = true;
  } else {
    latestEcgValue = 0;   // zero signals leads-off to downstream logic
    newSampleAvailable = true;
  }
  portEXIT_CRITICAL_ISR(&ecgValueMux);
}

// ============================================================
// TIMER ISR — 360 Hz sample collector
// ============================================================
// The ADS1292R ODR is set to 500 SPS. The DRDY ISR updates
// latestEcgValue at 500 Hz. This timer fires at 360 Hz and
// snapshots the most-recent latestEcgValue into the ping-pong
// buffer, providing a clean 360 Hz stream to the rest of the
// pipeline (same rate as the original AD8232 firmware).
void IRAM_ATTR onTimer() {
  portENTER_CRITICAL_ISR(&timerMux);

  int32_t sample;
  portENTER_CRITICAL_ISR(&ecgValueMux);
  sample = latestEcgValue;
  portEXIT_CRITICAL_ISR(&ecgValueMux);

  buffers[activeBuffer][sampleIndex] = (uint16_t)sample;
  sampleIndex++;
...
void setup() {
  ...
  // SPI.begin() MUST be called before any SPI.transfer() or library init.
  // The ProtoCentral library does NOT call SPI.begin() internally.
  // Default ESP32 SPI pins: MOSI=23, MISO=19, SCK=18.
  SPI.begin();
  SPI.setFrequency(4000000);   // ADS1292R max SPI clock = 4 MHz
  SPI.setDataMode(SPI_MODE1);  // ADS1292R: CPOL=0, CPHA=1
  ...
  pinMode(ADS1292_START_PIN, OUTPUT);  digitalWrite(ADS1292_START_PIN, LOW);
  pinMode(ADS1292_PWDN_PIN,  OUTPUT);  digitalWrite(ADS1292_PWDN_PIN, HIGH);

  // Power-on reset sequence per ADS1292R datasheet Section 9.5
  digitalWrite(ADS1292_PWDN_PIN, LOW);   delay(100);
  digitalWrite(ADS1292_PWDN_PIN, HIGH);  delay(100);

  // Let library configure registers (gain, ODR, lead-off, channel modes)
  ADS1292R.ads1292Init(ADS1292_CS_PIN, ADS1292_PWDN_PIN, ADS1292_START_PIN);
  Serial.println("[ADS1292R] Initialized — SPI @ 4MHz, Mode1, MSB-first");

  // Attach DRDY falling-edge interrupt AFTER sensor is fully initialised.
  // The ISR reads SPI — SPI must be ready before any DRDY fires.
  attachInterrupt(digitalPinToInterrupt(ADS1292_DRDY_PIN), onDRDY, FALLING);
  Serial.println("[ADS1292R] DRDY interrupt attached on GPIO " + String(ADS1292_DRDY_PIN));
...
void loop() {
  handlePowerButton();
  // NOTE: ADS1292R data acquisition is now fully interrupt-driven via onDRDY().
  // The loop() no longer needs to poll DRDY or touch SPI at all for ECG data.
  // The timer ISR snapshots latestEcgValue (written by onDRDY) at 360 Hz.
