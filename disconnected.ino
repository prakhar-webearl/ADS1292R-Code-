...
#define LEAD_OFF_DEBOUNCE_BLOCKS 2
...
uint8_t leadOffConsecutiveBlocks = 0;
bool leadOffLatched = false;
...
void sendTask(void* param) {
  ...
  for (;;) {
    if (xQueueReceive(blockQueue, &blk, portMAX_DELAY) == pdTRUE) {
      ...
      if (blk.lo) {
        if (leadOffConsecutiveBlocks < 255) leadOffConsecutiveBlocks++;

        if (leadOffConsecutiveBlocks >= LEAD_OFF_DEBOUNCE_BLOCKS) {
          if (!leadOffLatched) {
            leadOffLatched = true;
            resetAnalysisWindow();
          }
          condition = "Lead-Off Detected";
          severity = "CRITICAL";
          detail = "electrodes disconnected - check lead placement";
        } else {
          condition = "Lead status unstable";
          severity = "INFO";
          detail = "verifying disconnect";
        }
      } else {
        if (leadOffConsecutiveBlocks > 0) {
          leadOffConsecutiveBlocks = 0;
        }

        if (leadOffLatched) {
          leadOffLatched = false;
          resetAnalysisWindow();
          condition = "Leads Reconnected";
          severity = "INFO";
          detail = "signal reacquired, warming up";
        } else if (copyAnalysisWindow(analysisSnapshot, sampleCount, latestSeq)) {
