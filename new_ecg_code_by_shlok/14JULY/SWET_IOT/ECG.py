...
class ECGFilter:
    ...
    def _build(self):
        nyq = self.nyquist
        # 0.05 Hz high-pass (ultra-low cutoff to prevent ringing/undershoot)
        self.b_hp, self.a_hp       = butter(2, 0.05 / nyq, btype='high')
    ...
    def filter_signal(self, raw_signal):
        ...
        # Threshold increased to 500k to ensure legitimate sharp R-peaks are not chopped off.
        for i in range(1, len(sig) - 1):
            if abs(sig[i] - sig[i-1]) > 500000:
                sig[i] = (sig[i-1] + sig[i+1]) / 2.0

        # Zero-phase double median filter for perfect baseline wander removal without ringing
        # 1. 200ms median filter (removes QRS and narrow peaks)
        baseline = median_filter(sig, size=int(0.2 * self.fs))
        # 2. 600ms median filter (removes T-waves and smooths the baseline)
        baseline = median_filter(baseline, size=int(0.6 * self.fs))
        
        # Subtract the baseline wander
        sig = sig - baseline

        try:
            # Skip the IIR high-pass filter since baseline is already removed
...
def _annotate_one_beat(ax, sig, r_peak):
    ...
    local_window = sig[max(0, r_peak - 8):min(n, r_peak + 9)]
    ...
    ax.annotate('R', xy=(r_peak, sig[r_peak]),
                xytext=(r_peak, sig[r_peak] + 0.18 * local_amp),
    ...
    if p_hi > p_lo + 2:
        ...
        if p_idx is not None:
            ...
            ax.annotate('P', xy=(p_idx, sig[p_idx]),
                        xytext=(p_idx, sig[p_idx] + 0.12 * local_amp),
    ...
    if q_hi > q_lo + 2:
        ...
        if q_idx is not None:
            ...
            ax.annotate('Q', xy=(q_idx, sig[q_idx]),
                        xytext=(q_idx, sig[q_idx] - 0.14 * local_amp),
    ...
    if s_hi > s_lo + 2:
        ...
        if s_idx is not None:
            ...
            ax.annotate('S', xy=(s_idx, sig[s_idx]),
                        xytext=(s_idx, sig[s_idx] - 0.14 * local_amp),
...
def _process_block(blk):
    ...
    if lo_flag or all(int(v) == 0 for v in samples) or "LEADS_OFF" in active_warnings:
        electrode_status = 'LEADS_OFF'
        leads_off_events.append(time.time())
    elif all(abs(int(v)) > 7_500_000 for v in samples) or any("Flatline" in w or "Noise" in w for w in active_warnings):
...
def update(frame):
    ...
    try:
        ...
        if len(rp_local) >= 2:
            ...
            _annotate_one_beat(ax_filt, filtered_array, rp_local[center_idx])
    except Exception:
        pass
