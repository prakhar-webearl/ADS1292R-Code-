 ...
 *  - /wifi-config POST endpoint: mobile app sends credentials → ESP
 *    connects to new network, saves creds, stops AP, LED turns ON.
 *  - STA drops: reconnects every 5s. After 3 fails → starts AP.
...
// Call every loop() — handles reconnect timers, AP fallback,
...
bool network_uploadBlock(const Block& blk, bool leadsOff, bool loPlus, bool loMinus, const char* warning = nullptr);
